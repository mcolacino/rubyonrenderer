...
class TemplateTest < Test::Unit::TestCase
  ...
  def test_placeholders_counting
    @template = Renderer::Template.new %Q{<div class="titolo">{titolo}</div>}
    # Un template con un unico ph
    assert_equal 1, @template.placeholders.size
    # Un template con due placeholder
    @template2 = Renderer::Template.new %Q{<div class="titolo">{titolo} <span>{data}</span></div>}
    ...
    @template3 = Renderer::Template.new %Q{<div class="titolo">{titolo} {WrongId}<span>{data}</span></div>}
  ...
  def test_adding_a_new_placeholder_invalid_becouse_included_in_reserved_words
    assert_raise InvalidNameForPlaceholder do
      @template = Renderer::Template.new %Q{<div class="titolo">{format}</div>}
    end
  end
  ...
  def test_removing_a_non_existing_placeholder
    @template = Renderer::Template.new ''
    # Un template con un unico ph
  ...
  def test_dynamic_accessors_created_on_placeholder
    ...
    @template1 = Renderer::Template.new %Q{<div class="titolo">{titolo}</div>}
    assert @template1.respond_to? 'titolo'
    @template1.add_placeholder 'data'
    assert @template1.respond_to? 'data'
    @template2 = Renderer::Template.new %Q{<div class="titolo">{titolo}</div>}
